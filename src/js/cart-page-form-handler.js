...
export function CartPageFormHandler($, fo) {
    ...
    this.processFormData = function (stateAsync, self = this) {
        ...
        this.updateCountryAndState.call(this,stateAsync, this.countryAddressObj.value);
    };

    this.updateCountryAndState = function (stateAsync, country_value) {
        const resFnc = this.setCountryAndStateInputField.bind(this)
        stateAsync(country_value).then(
            resFnc
        ).catch(
    ...
    /**
     * Updates the State input field
     * @param {Object} stateObject JS object containing the state list
     * @param {string} name_attr The value of the name attribute of the state input filed
     */
    this.updateStateEl = function(stateObj=null){
        if(stateObj){
            this.setStates(stateObj);
        }
